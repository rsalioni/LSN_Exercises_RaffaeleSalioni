...
//measurement of g(r)
  ...
  timestep=0; //contatore step totali
...
// contribution to energy and virial
  ...
  if(printstep == 1){
  	timestep++;
  	ofstream v, p;
  	v.open("output.epot.dat", ios::app);
  	p.open("output.pres.dat", ios::app);
  	v << timestep << ", " << walker[iv]/npart << endl;
  	p << timestep << ", "  << rho * temp + (walker[iw] + ptail * (double)npart) / vol << endl;;
		v.close();
		p.close();
	}  
