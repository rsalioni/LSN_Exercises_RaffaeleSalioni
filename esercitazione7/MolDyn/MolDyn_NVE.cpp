...
void Blocchi(int nconf){
	...
	if(nconf%((iprint/nblocchi)) == 0){
		for(int i=0; i<m_props-nbins; i++){
			//cout << "media" << i <<" " << sum[i]/(iprint/nblocchi) << endl;   //(iprint/nblocchi)
			//f[i].open("outputblocchi_" + to_string(i) + ".dat", ios::app);
		...
		/* //old
		for(int k=4; k<n_props; k++){
			//r = (k-4)*bin_size;
			gdir = sum[k]/(iprint/nblocchi);		
			ave[k] += gdir;
			ave2[k] += gdir*gdir;
		...
		*/
		...
		if(nconf == iprint){
			cout << "stampo gave " << nconf << endl;
			ofstream Gave;
			Gave.open("output.gave.0",ios::app);
			for(int k=4; k<n_props; k++){
				r = (k-4)*bin_size;
				norm = 1./((4./3.)*pi*(pow((r + bin_size),3) - pow(r,3))*rho*npart);
				//Gave <<  r + bin_size/2.<< ", " << norm*ave[k]/(double)nblocchi << ", " << norm*Error(ave[k],ave2[k],nblocchi) << endl;
				Gave <<  r + bin_size/2.<< ", " << norm*ave[k]/(nconf/(iprint/nblocchi)) << ", " << norm*Error(ave[k]/(nconf/(iprint/nblocchi)),ave2[k]/(nconf/(iprint/nblocchi)),nconf/(iprint/nblocchi)) << endl;
			}
			Gave.close();
		}
	}
