...
int main (int argc, char *argv[]){
	...
	Posizione p(1.5, 0., 0.); //Posizione p(1.5, 0., 0.)
	...
	double ave2=0.;
	int k = 0;
	ofstream fout1("rmedia1S.txt");
	ofstream fout2("pos1S.txt");
	//ofstream fout3("r_eq.txt"); //equilibrazione
	...
	for(int i=0; i<nblocchi; i++){ //ciclo sui blocchi
		...
		ave += sum/n;
		ave2 += pow(sum/n, 2);
		fout1 <<  (i+1)*n << ", " << ave/(i+1) << ", " << error(ave/(i+1), ave2/(i+1), i+1) << endl;
  		cout <<  (i+1)*n << ", " << ave/(i+1) << ", " << error(ave/(i+1), ave2/(i+1), i+1) << endl;
	}
