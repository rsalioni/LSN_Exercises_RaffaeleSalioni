...
public:
	...
	void Metropolis_gauss(FunzioneScalareBase* f);
...
class MediaBlocchi { //aggiungere appo

public:
	//costruttore
	MediaBlocchi();
	MediaBlocchi(double nblocchi, double ntot);
	//metodi
	int getN() const {return m_ntot;};
	int getNb() const {return m_nblocchi;};
	int getNperB() const;
	
	void setN( double n) { m_ntot = n;};
	void setNb( double nb) { m_nblocchi = nb;}; 
	
	void Blocchi( double data, int step);
	double getMean(int step);
	double getError(int step);
	 
private:
	int m_nblocchi;
	int m_ntot;
	double m_sum, m_ave, m_ave2;
