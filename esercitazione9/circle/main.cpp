...
vector<vector<int>> New_Pop(vector<vector<int>> v){
	...
	for(int l=0; l<NN/2; l++){
		...
		if(my_rand.Rannyu() < 0.75){//probabilità crossover
...
int main (int argc, char *argv[]){
	...
	ofstream fout("L2_circle.txt");
	ofstream fout2("path_circle.txt");
	ofstream fout3("ave_circle.txt");
	...
	*/
	for(int i=0; i<32; i++){
		city = Pop[0][i];
		cout << city << " ";
		fout2 << cos(theta[city-1]) << ", " << sin(theta[city-1]) << endl;
	}
	 fout.close();
	 fout2.close();
	 fout3.close();
